...
졸업요건을 확인하려면 **교과과정 책자를 영역별로 직접 찾아가며 대조**해야 했습니다.

- 교양 필수 과목 목록을 책자에서 찾아 하나씩 확인
- 드래곤볼(영역별 교양) 7개 영역 중 몇 개를 채웠는지 수동 집계
- 전공 필수 과목 이수 여부를 책자에서 별도 확인
- MSC(수학·과학·전산) 요건은 학과마다 기준이 달라 더 복잡

클래스넷에서 이수내역을 보더라도, 각 영역의 기준과 내 이수 현황, 졸업까지 남은 학점을 직접 비교하는 작업은 **귀찮고 불편한 작업** 이었습니다.

### 해결 방안

클래스넷 성적표 텍스트를 붙여넣기만 하면, 교양·전공·MSC 모든 졸업요건을 **한 번에 자동으로 대조해서 충족 여부를 즉시 확인**할 수 있도록 했습니다.
